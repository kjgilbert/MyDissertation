...
By parsing total fitness into each of its quantitative trait and deleterious loci fitness components, we then further investigated the mechanism of fitness reduction during range expansion. Not unexpectedly, we found that increasing the steepness of the gradient increases migration load in populations. In the cases for U = 0.0, migration load, and any expansion load due to maladaptive quantitative locus mutations are the only contributing factors to reduced fitness. Dispersal mode was not a significant factor in reducing fitness, but environmental gradient, presence of deleterious mutations, and their interaction all contributed to fitness at the range edge in just the quantitative trait. (\emph{but the delet muts aren't a straightforward interaction b/c their presence actually improves quanti fitness - probably need to work on this section because of this point})
Similar results were shown when examining only the fitness due to deleterious mutations -- gradient and rate of mutations impacted fitness.
As can be seen in Figure \ref{fig:load}, the effects on fitness are not always to decrease it at the edge. Increasing deleterious mutation rate over all types of gradients decreases fitness, but within each mutation rate case, a steeper gradient lessens the load due to deleterious mutations. For quantitative traits, steeper gradients do always reduce fitness, but having higher deleterious mutation rates lessens this load.
