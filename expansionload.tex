...
As expected, we found that increasing the steepness of the gradient increases migration load in populations (Figure \ref{fig:load}). This is clearly seen within each mutation case (\emph{do stats that class the same gradient across mut params into 1, or keep all separate?}). This slows the speed of expansion within each mutation class (Figures \ref{fig:speed}). The leptokurtic dispersal kernel resulted in faster expansion over the landscape (\emph{do stats gauss vs lepto}). Increasing the deleterious genomic mutation rate slightly (run these stats!) decreases the speed of expansion within each environmental gradient class.

We find an interaction of environmental gradient with the presence of deleterious mutations, where
