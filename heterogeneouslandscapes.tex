...
The quantitative trait, $z$ is determined by 100 freely recombining loci. The selection variance, $V_S$, was set at 7.5, while environmental variance was set at 1. Selection is stabilizing for the phenotypic optimum of a given cell, where the fitness of a trait value, $z$ is given by:
 $\omega_z = exp[-\frac{(z-z_{opt})^2}{2V_S}]$%$W(z) = exp[-\frac{1}{2}(z - \theta)^T\omega^{-1}(z-\theta)]$
  where $z_{opt}$ is the environmental optimum.%where $\theta$ is the environmental optimum, and $\omega$ is the ``variance-covariance matrix of selection describing the individual fitness surface``, according to \textsc{nemo}. 
...
%We would like to thank Fredéric Guillaume for help with \textsc{nemo} and members of the Whitlock lab for feedback on the project.
