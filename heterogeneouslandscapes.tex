...
For the same gradient values and genetic architectures, we examined the impact for dispersal distances to assess the ability for range expansion and the value of $b_{crit}$. The predicted $b_{crit}$ for these scenarios is approximately half of those for the smaller dispersal distance ($\approx 0.082, 0.063, 0.009,$ and $0.0008$, respectively). %%; values which are approximately half of the predictions for the smaller dispersal kernel. 
...
We now examine how the outcome of range expansion changes as landscape heterogeneity changes. We model two values of the overall gradient ($b = 0.0075$ and $b = 0.015$), both of which create a very weak linear gradient for which expansion is easily successful. Holding this total gradient constant, we decrease the number of patches across the landscape at a range from $1960$ (linear) to $2$ and increase $\Delta z_{opt}$. Within a given simulation, $\Delta z_{opt}$ is constant, but across simulations $\Delta z_{opt}$ varies as the number of patches varies, i.e. $b = \Delta z_{opt}(number~of~patches-1)$. 

On the weaker gradient ($b = 0.0075$), expansion occurs in all cases except the two patch model ($\Delta z_{opt} = 15$). One replicate did expand in this scenario for the medium-small regime after a $10,000$ generation delay, but none of the remaining 79 replicates succeeded in expanding into a second patch at this value of $\Delta z_{opt}$. The medium-large regime showed a significantly lower rate of growth than the other three regimes in the case of four patches on the landscape ($\Delta z_{opt} = 7.5$; Figure \ref{fig:hetspeed}). On the stronger gradient ($b = 0.015$), the same patterns of expansion were found when comparing across $\Delta z_{opt}$ values but not when comparing across the number of patches on the landscape. In other words, on the steeper overall environmental gradient, the ability to expand was driven by the local change in the environment, $\Delta z_{opt}$, not by the number of patches that the landscape was divided into, nor the steepness of the overall gradient, as this was constant. The only difference on the steeper gradient is within the medium-large regime where population growth rate is slower in all increasing numbers of patches (decreasing values of $\Delta z_{opt}$), not only when $\Delta z_{opt} = 7.5$ as was the case on the weaker overall gradient (Figure \ref{fig:hetspeed}) This leads us to next test if $\Delta z_{opt}$ can predict expansion.
...
\centering
\makebox[\textwidth]{
        \includegraphics[width=0.8\linewidth]{Figures/GrowthRate_ByStepNum.pdf}}
\caption[Growth rate of populations expanding over a patchy gradient.]{\color{red}NB missing points are sims still running. I could alternatively lay this figure out like Figure \ref{fig:multistep}? \color{black} Growth rate of populations expanding over a patchy gradient for $b = 0.0075$ (circles) and $b = 0.015$ (triangles). The four different regimes of genetic architecture are indicated along the $x$-axis. Each cluster of points from left to right order the simulations by the number of patches on the landscape from $2$ to $1960$. Cases where $\Delta z_{opt}$ is equal to that of the two patch model in Figure \ref{fig:waittimes} are colored equivalently. All points in grayscale have a $\Delta z_{opt}$ less than and decreasing below $4$. The asterisk indicates the case where a single replicate that expanded contributes to this mean growth rate. This upper $95\%$ confidence interval spans to a growth rate of $91$ individuals per generation. Error bars indicate 95\% confidence intervals.}
\label{fig:hetspeed}
\end{figure}
...
\caption[Time spent before expansion into second patch.]{Time spent (in generations) before expansion into second patch for each genetic architecture regime. Points are jittered for visualization. All points beyond the vertical dashed line are censored and did not expand during the course of the simulation. \color{red}Do you think I should combine this figure with Figure \ref{fig:constVG} into just one figure?\color{black}}
\label{fig:waittimes}
\end{figure}
...
To examine if the amount of genetic variance, instead of mutational effect sizes drives the ability for populations to expand and adapt, we analyze a subset of simulations that held $V_G$ constant instead of $V_M$. With a decreasing $\alpha^2$, expansion became more difficult. Expansion no longer occurred for equivalent values of $\Delta z_{opt}$ and became more difficult at lower values of $\Delta z_{opt}$ (Figure \ref{fig:constVG}). This shows that genetic variance was not the determining factor for expansion in these cases, but that the availability of mutations of larger effect determined the ability to adapt and expand into the second patch. %%%% FILL IN 316 and 1000 loci results - AND NEW CONST VG RESULTS!!!
...
Our study has investigated two major aspects of range expansion biology: how do landscape heterogeneity and genetic architecture change the outcome of adaptation and expansion. We find that both of these factors can impact the ability for populations to expand. Landscape heterogeneity shows a complex result where moving away from the realm of pure linear gradients or pure two patch models makes the ability for predicting a successful range expansion impossible from knowing either the overall gradient value or the degree of change in environment between patches. Genetic architecture shows an interesting pattern where adaptation and expansion can succeed from either many mutations of small effect or few of large effect, but that intermediate between these two regimes, adaptation becomes more difficult or even impossible.

The genetic architecture of the system impacted range expansion in all cases of landscapes investigated, from linear gradients to two patch models to heterogeneous gradients consisting of many patches. In some cases genetic architecture changed the outcome between failure or success to expand, but in other cases only affected the speed at which range expansion proceeded and the fitness of populations during this expansion. %had something else in mind to maybe say here, just slipped my mind
Though genetic variance has been used previously for determining the critical steepness of environmental gradients for which expansion could or could not succeed \citep{Kirkpatrick:1997, Barton:2001, Bridle:2010, Polechova:2015}, we find that genetic variance is not always sufficient for describing this outcome. With equivalent genetic variance, the availability of larger effect mutations can lead to successful range expansion that is not possible via small effect alleles. % say something about genetic variance being higher on steeper gradients? maybe not, it's more a product of migration load there
The genetic architectures we investigated revealed two qualitatively different scenarios for which adaptation to newly encountered environments can proceed. First, sufficient numbers of small effect alleles can lead to successful expansion, and second, few alleles of sufficiently large effect can also lead to successful expansion. In the intermediate regimes between these scenarios, populations can lack either of these factors, slowing or preventing range expansion. 



The structure of the landscape over which range expansion occurs has a large impact on the outcome of expansion. The success of expansion over a linear gradient was predicted by the steepness of the gradient, $b$. The values of $b$ which prevented expansion were much higher than what is predicted using the formulate for $b_{crit}$ from \citet{Barton:2001}. There are several major reasons for which these predictions are not expected to match our measured $b_{crit}$. Our two-dimensional landscape does not perfectly match the continuous space simulations from which these predictions derive. Perhaps more importantly, our simulations begin with populations that have been burned in to a landscape core to which they are well adapted before the opportunity to expand is presented to them. Previous studies initiate their populations on the environmental gradient, whereas ours begin in a core patch with a constant environmental optimum, and only outside of this core does the environment change. Therefore, even if a difficult gradient is presented, global extinction cannot occur because the core will survive. Cases where a gradient may be too steep for a population to persist long enough in order to adapt is not an issue for our model because individuals dispersing from the core will continually be able to attempt adapting to the new environment throughout the course of the simulation without being subject to extinction. 
...
Furthermore, predicting expansion based on values of genetic variance was not useful, as even for cases of equal genetic variance but varying genetic architecture, the outcome of expansion differed. Genetic variance evolves differently on each gradient and alone is not a sufficient predictor of expansion. We did observe an upper limit to genetic variance, above which populations did not exist, presumably due to the lack of local adaptation as individuals are too different from their local phenotypic optimum, reducing population density and fitness to non-sustainable levels.

We have clearly shown that increasing the patchiness across an environmental gradient can limit range expansion. Predicting where this patchiness creates a limit for range expansion is complicated. The change in environmental optimum between two patches contributes strongly to the ability to continue expanding or not, but is not the only characteristic of the landscape that matters. When more patches are present, the degree of change between them must be lower than it could be to result in expansion over fewer patches. This is attributed to the effects of migration load on local adaptation within patches. As more patches are present, migrants from different patches are encountered more often as mates, and the influx of locally maladaptive alleles reduces population fitness. At a range edge, this can make adapting to a new environment more difficult as individuals are not even fully adapted to their local patch since the overall gradient is effectively steeper. This is similar to the result found by \citet{GarciaRamos:1997} where edge populations are pulled closer to the optimum of the denser, more central populations. Investigating the effects of migration barriers on heterogeneous landscapes as well as landscapes that do not always increase monotonically holds value. Moreover, incorporating the evolution of dispersal ability may produce different results as populations adapt their dispersal ability to better match the level of patchiness in their environment and limit the detrimental effects of migration load.
...
\caption[Time spent before expansion into second patch.]{Time spent (in generations) before expansion into second patch for each genetic architecture regime with a larger dispersal kernel of $2\sigma_{disperse}$. Points are jittered for visualization. All points beyond the vertical dashed line are censored and did not expand during the course of the simulation.}
