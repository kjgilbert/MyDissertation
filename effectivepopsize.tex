...
an isolation by distance (IBD) scenario. Figure 1 shows the design for the island model cases 
...
and estimation of \emph{N}$_e$ for these demographic cases (Fig. 1 for the island model) is meant 
...
labeled incorrectly as a migration source (Fig. 1). This allowed us to further assess whether 
...
and the supplemental material. Figure 2 shows the RMSE of all methods for the isolation (no migration) 
...
large RMSE at 50 and 500 (Fig. 2A). \textsc{mlne}'s likelihood estimate performs next best 
...
the isolation cases (Fig. 2). Note that the y-axes on Figure 2B and C have a greater range than 
in Figure 2A and remain on a log scale. \textsc{neestimator}'s coancestry and heterozygote excess 
...
\begin{table}[]
\centering
\begin{tabular}{p{0.2\textwidth}|p{0.3\textwidth}|p{0.6\textwidth}}
